...
class PosController extends Controller
{
    ...
    public function checkout(Request $request, Order $order)
    {
        ...
        $clientName = $clientId ? Client::find($clientId)->name : ($request->input('client_name') ?? 'Consumidor Final');
        // Tipos de documento (Colombia / DIAN):
        // - Ticket: venta interna no electrónica
        // - Factura: Factura Electrónica de Venta (DIAN, tipo 01)
        $documentType = $request->input('document_type', 'Ticket');
        $clientDocument = $request->input('client_document');

        // Validación para Factura Electrónica de Venta DIAN:
        // requiere identificación del adquiriente y razón social/nombre.
        if ($documentType === 'Factura') {
            $doc = preg_replace('/\D/', '', (string) $clientDocument);
            if (strlen($doc) < 6) {
                return redirect()->back()->with('error', 'Para emitir Factura Electrónica el cliente debe tener un documento de identificación válido (NIT o CC).');
            }
            if (empty(trim((string) $clientName)) || $clientName === 'Consumidor Final' || $clientName === 'Público') {
                return redirect()->back()->with('error', 'Para emitir Factura Electrónica debe indicar la razón social o nombre del adquiriente.');
            }
        }

        DB::transaction(function() use ($order, $method, $received, $change, $request, $clientId, $clientName, $documentType, $clientDocument) {
            // 1. Calcular IVA (Colombia: tarifa general 19%) si es comprobante electrónico
            $config = new DianConfig();
            $ivaFactor = $config->ivaFactor();   // 0.19 por defecto
            $isElectronic = $documentType === 'Factura';

            $subtotal = 0;
            $iva = 0;
            if ($isElectronic) {
                // Los precios del POS incluyen IVA → desagregar
                $subtotal = round((float) $order->total / (1 + $ivaFactor), 2);
                $iva      = round((float) $order->total - $subtotal, 2);
            }

            // 2. Asignar prefijo y número consecutivo DIAN si es electrónico
            $prefijo = null;
            $numero = null;
            if ($isElectronic) {
                $next = DianResolution::nextNumber();
                $prefijo = $next['prefijo'];
                $numero  = $next['numero'];
            }
            ...
            $order->update([
                ...
                // DIAN
                'dian_prefijo' => $prefijo,
                'dian_numero'  => $numero,
                'subtotal'     => $subtotal,
                'iva'          => $iva,
                'dian_status'  => $isElectronic ? 'PENDING' : null,
            ]);
        ...
        // 3. Envío a DIAN (no bloqueante: si falla queda en ERROR y puede reintentarse desde Billing).
        if ($order->isElectronic()) {
            try {
                (new DianService())->sendInvoice($order->fresh('details.product'));
            } catch (\Throwable $e) {
        ...
        if ($order->isElectronic()) {
            $order->refresh();
            $msg .= ' Factura ' . $order->full_number . ' - ' . ($order->dian_description ?? $order->dian_status);
        }
